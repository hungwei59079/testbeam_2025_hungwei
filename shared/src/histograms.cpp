                         ...
                         const std::string& output_root_file)
{
    ...
    // handle fixed-length arrays or scalars
    if (leaf_type == "Float_t") {
        Float_t value[1024] = {0};
	...
	Double_t value[1024] = {0};
        ...
        for (Long64_t i = 0; i < n_entries; ++i) {
            ...
            std::cerr << "Unsupported non-vector type: " << leaf_type << std::endl;
        }
    }
